...
if [ $? != 0 ] ;then
  ...
  hdfs dfs -chown "$HADOOP_USER":"$HADOOP_USER" /user/"$HADOOP_USER"
fi
echo -e "HDFS_DIR is ${HDFS_DIR}"

hdfs dfs -ls /user/"$HADOOP_USER"/${HDFS_DIR}
if [ $? != 0 ] ;then
  hdfs dfs -mkdir /user/"$HADOOP_USER"/${HDFS_DIR}
fi
