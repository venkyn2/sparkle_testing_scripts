...
$SPARK_HOME/bin/spark-submit --class  com.github.ehiggs.spark.terasort.TeraSort --master $SPARK_MASTER_URL --conf spark.network.timeout=40000 --conf spark.rpc.numRetries=5 --conf spark.yarn.executor.memoryOverhead=600 --num-executors=$SPARK_NUM_EXECUTOR --conf "spark.cores.max=$SPARK_MAX_CORE" --conf spark.dynamicAllocation.enabled=false --conf "spark.local.dir=/data1" --conf "spark.executor.extraJavaOptions=-Djava.io.tmpdir=/data1" --conf "spark.driver.extraJavaOptions=-Djava.io.tmpdir=/data1" --conf spark.default.parallelism=$SPARK_DEFAULT_PARALLEL --conf "spark.executor.extraJavaOptions=-XX:-UseBiasedLocking" --conf "spark.driver.extraJavaOptions=-XX:-UseBiasedLocking" --executor-memory=$SPARK_EXECUTOR_MEMORY --executor-cores=$SPARK_EXECUTOR_CORE   ${SPARK_TERASORT_JAR} $HDFS_DIR/HSsort-input  $HDFS_DIR/HSsort-output  &> $LOGFILE

#$SPARK_HOME/bin/spark-submit --class  com.github.ehiggs.spark.terasort.TeraSort --master $SPARK_MASTER_URL --num-executors=$SPARK_NUM_EXECUTOR --conf "spark.cores.max=$SPARK_MAX_CORE" --conf spark.dynamicAllocation.enabled=false --conf "spark.local.dir=/data1" --conf "spark.executor.extraJavaOptions=-Djava.io.tmpdir=/data1" --conf "spark.driver.extraJavaOptions=-Djava.io.tmpdir=/data1" --conf spark.default.parallelism=$SPARK_DEFAULT_PARALLEL --conf "spark.executor.extraJavaOptions=-XX:-UseBiasedLocking" --conf "spark.driver.extraJavaOptions=-XX:-UseBiasedLocking" --executor-memory=$SPARK_EXECUTOR_MEMORY --executor-cores=$SPARK_EXECUTOR_CORE   ${SPARK_TERASORT_JAR} $HDFS_DIR/HSsort-input  $HDFS_DIR/HSsort-output  &> $LOGFILE
